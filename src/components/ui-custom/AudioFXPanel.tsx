...
import { EQ_PRESETS, EQPreset } from '@/hooks/useAudioFX';
...
const AudioFXPanel: React.FC<AudioFXPanelProps> = ({ compact = false }) => {
  const {
    fxState: state,
    toggleEQ, setEQPreset, setEQBandGain,
    setVolumeBoost, toggleBassBoost, setBassGain,
  } = useAudio();
      ...
      <div className="space-y-3 p-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {state.eqEnabled && presetKeys.map(key => (
            <Button key={key} variant={state.preset === key ? 'default' : 'outline'} size="sm"
              className="h-6 px-2 text-[10px] rounded-full" onClick={() => setEQPreset(key)}>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 flex-1">
            ...
            <Slider value={[state.bassBoostGain]} onValueChange={(v) => setBassGain(v[0])} max={12} step={0.5} className="flex-1" disabled={!state.bassBoostEnabled} />
            <span className="text-[10px] text-muted-foreground font-mono w-8 text-right">{state.bassBoostEnabled ? `+${state.bassBoostGain}` : 'Off'}</span>
          </div>
          <div className="flex items-center gap-2 flex-1">
            ...
            <Slider value={[state.volumeBoost]} onValueChange={(v) => setVolumeBoost(v[0])} min={0} max={200} step={5} className="flex-1" />
            <span className="text-[10px] text-muted-foreground font-mono w-8 text-right">{state.volumeBoost}%</span>
          </div>
        </div>
      </div>
    );
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md space-y-4">
      <div className="p-4 rounded-xl bg-card/30 backdrop-blur-sm border border-border/30">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {presetKeys.map(key => (
            <Button key={key} variant={state.preset === key ? 'default' : 'outline'} size="sm"
              className={`h-7 px-3 text-xs rounded-full ${state.preset === key ? 'shadow-md' : 'opacity-70 hover:opacity-100'}`}
              onClick={() => setEQPreset(key)} disabled={!state.eqEnabled}>
              {EQ_PRESETS[key].label}
            </Button>
          ))}
          {state.preset === 'custom' && (
            <Button variant="default" size="sm" className="h-7 px-3 text-xs rounded-full shadow-md" disabled>Custom</Button>
          )}
        </div>
        {state.eqEnabled && (
              ...
              <div key={i} className="flex flex-col items-center flex-1 h-full">
                <div className="flex-1 w-full flex items-center justify-center relative">
                  <input type="range" min={-12} max={12} step={0.5} value={gain}
                    ...
                    style={{ writingMode: 'vertical-lr', direction: 'rtl', width: '20px', background: 'transparent' }} />
                  <div className="absolute bottom-0 w-1.5 rounded-full bg-muted overflow-hidden" style={{ height: '100%' }}>
                    <div className="absolute w-full bg-primary rounded-full transition-all"
                      style={{ height: `${Math.abs(gain) / 12 * 50}%`, bottom: gain >= 0 ? '50%' : `${50 - Math.abs(gain) / 12 * 50}%` }} />
                  </div>
                </div>
      ...
      <div className="p-4 rounded-xl bg-card/30 backdrop-blur-sm border border-border/30">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={toggleBassBoost}
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] text-muted-foreground font-mono">{state.bassBoostEnabled ? `+${state.bassBoostGain}dB` : 'Off'}</span>
            </div>
            <Slider value={[state.bassBoostGain]} onValueChange={(v) => setBassGain(v[0])} max={12} step={0.5} disabled={!state.bassBoostEnabled} />
          </div>
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl bg-card/30 backdrop-blur-sm border border-border/30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <Slider value={[state.volumeBoost]} onValueChange={(v) => setVolumeBoost(v[0])} min={0} max={200} step={5} />
          </div>
        </div>
      </div>
    </motion.div>
