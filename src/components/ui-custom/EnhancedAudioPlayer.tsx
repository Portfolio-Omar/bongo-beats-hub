...
import { 
  ...
  Repeat, Shuffle, Heart, ChevronUp, Disc, AudioLines
} from 'lucide-react';
...
const EnhancedAudioPlayer: React.FC = () => {
  ...
  // Bass boost
  const { bassBoost, toggleBassBoost, setBassGain } = useBassBoost(audioRef.current);
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="absolute top-0 left-0 right-0 h-1 bg-muted">
          <motion.div className="h-full bg-primary" style={{ width: `${progressPercentage}%` }} />
        </div>
          ...
          {showBassControls && (
            ...
            >
              <div className="px-4 py-3 flex items-center gap-3">
                <AudioLines className={`h-4 w-4 flex-shrink-0 ${bassBoost.enabled ? 'text-primary' : 'text-muted-foreground'}`} />
                <span className="text-xs font-medium min-w-[50px]">Bass</span>
                <Slider
          ...
          <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
            ...
            >
              ...
              <Button
                variant="ghost"
                size="icon"
                className="h-7 w-7 rounded-full md:hidden flex-shrink-0"
                onClick={(e) => { e.stopPropagation(); handleOpenPlayer(); }}
              >
            ...
            <div className="flex items-center gap-0.5 sm:gap-1 md:gap-2">
              ...
              <Button
                ...
                onDoubleClick={(e) => {
                  e.stopPropagation();
                  setShowBassControls(!showBassControls);
                }}
                className={`h-7 w-7 sm:h-8 sm:w-8 rounded-full hidden sm:flex ${bassBoost.enabled ? 'text-primary bg-primary/10' : 'text-muted-foreground'}`}
              >
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button
                  ...
                  size="icon"
                  onClick={() => {
                    if (!showBassControls) setShowBassControls(true);
                    else toggleBassBoost();
                  }}
                  className={`h-6 w-6 rounded-full sm:hidden ${bassBoost.enabled ? 'text-primary' : 'text-muted-foreground'}`}
                >
