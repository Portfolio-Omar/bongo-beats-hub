      ...
      >
          ...
          {showSleepMenu && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden border-b border-border/30">
              <div className="flex items-center gap-2 p-2 px-4">
                ...
                <span className="text-xs font-medium mr-2">Sleep Timer:</span>
                {[15, 30, 45, 60].map(min => (
                  <Button key={min} variant={sleepTimer.active && sleepTimer.duration === min * 60 ? 'default' : 'outline'}
                    size="sm" className="h-6 px-2 text-[10px] rounded-full"
                    onClick={() => { setSleepTimer(min); setShowSleepMenu(false); toast.success(`Sleep timer set for ${min} minutes`); }}>
                    {min}m
                  </Button>
                ))}
                {sleepTimer.active && (
                  <Button variant="destructive" size="sm" className="h-6 px-2 text-[10px] rounded-full"
        ...
        <div className="p-2 sm:p-3 md:p-4">
          <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
            <motion.div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0 cursor-pointer" whileHover={{ scale: 1.02 }} onClick={() => navigate('/player')}>
              <motion.div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg overflow-hidden bg-muted flex-shrink-0"
                animate={{ rotate: isPlaying ? 360 : 0 }} transition={{ duration: 10, repeat: isPlaying ? Infinity : 0, ease: "linear" }}>
              ...
              <div className="min-w-0 flex-1">
                ...
                {sleepTimer.active && (
                  <p className="text-[9px] text-primary font-mono">⏱ {formatSleepTime(sleepTimer.remaining)}</p>
                )}
              </div>
            ...
            <div className="flex items-center gap-0.5 sm:gap-1 md:gap-2">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button size="icon" onClick={togglePlayPause} className="h-9 w-9 sm:h-10 sm:w-10 md:h-12 md:w-12 rounded-full bg-primary hover:bg-primary/90 shadow-lg">
              ...
              <Button variant="ghost" size="icon" onClick={() => setShowSleepMenu(!showSleepMenu)}
                className={`h-7 w-7 sm:h-8 sm:w-8 rounded-full ${sleepTimer.active ? 'text-primary bg-primary/10' : 'text-muted-foreground'}`}>
                {sleepTimer.active ? <TimerOff className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> : <Timer className="h-3 w-3 sm:h-3.5 sm:w-3.5" />}
              </Button>
            </div>
            ...
            <div className="hidden md:flex items-center gap-2 min-w-0 flex-1 max-w-xs justify-end">
              ...
              <div className="text-[10px] text-muted-foreground flex-shrink-0 font-mono ml-1">{formatTime(currentTime)} / {formatTime(duration)}</div>
            </div>
          </div>
          ...
          <div className="mt-1.5 md:hidden">
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              <div className="flex items-center gap-1">
                <Button variant="ghost" size="icon" onClick={toggleShuffle} className={`h-6 w-6 rounded-full sm:hidden ${isShuffled ? 'text-primary' : 'text-muted-foreground'}`}>
                  <Shuffle className="h-2.5 w-2.5" />
                </Button>
                <Button variant="ghost" size="icon" onClick={toggleRepeat} className={`h-6 w-6 rounded-full sm:hidden ${isRepeating ? 'text-primary' : 'text-muted-foreground'}`}>
