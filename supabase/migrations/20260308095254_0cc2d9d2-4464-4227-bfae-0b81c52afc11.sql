...
CREATE TABLE public.referrals (
  ...
  referred_songs_count integer NOT NULL DEFAULT 0,
...
CREATE POLICY "Users read own referrals as referrer" ON public.referrals FOR SELECT USING (auth.uid() = referrer_id);
CREATE POLICY "Users read own referral as referred" ON public.referrals FOR SELECT USING (auth.uid() = referred_id);
CREATE POLICY "Users insert referrals" ON public.referrals FOR INSERT WITH CHECK (auth.uid() = referred_id);
CREATE POLICY "Admin read all referrals" ON public.referrals FOR SELECT USING (true);

-- Add referral_code to user_earnings for easy lookup
ALTER TABLE public.user_earnings ADD COLUMN IF NOT EXISTS referral_code text UNIQUE;

-- Function to generate referral code on earnings creation
...
CREATE TRIGGER set_referral_code
  BEFORE INSERT ON public.user_earnings
  FOR EACH ROW
  EXECUTE FUNCTION public.generate_referral_code();

-- Update existing rows with referral codes
UPDATE public.user_earnings SET referral_code = substr(md5(user_id::text || now()::text), 1, 8) WHERE referral_code IS NULL;

-- Update process_listen_reward to track referral progress
...
BEGIN
  ...
  -- Referral tracking: increment referred user's song count
  UPDATE referrals SET referred_songs_count = referred_songs_count + 1
  WHERE referred_id = _user_id AND bonus_paid = false;

  -- Check if referral bonus should be paid (10 songs threshold)
